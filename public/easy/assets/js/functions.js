...
(function($){
	...
	SEMICOLON.header = {
		...
		menufunctions: function(){
		    ...
		    if($( '#primary-menu ul li:has(ul)' ).hasClass('sub-menu'))
                return;
		...
		filterInit: function(){
			...
			$portfolioFilter.each( function(){
				var element = $(this),
                    ...
                    elementContainer = $(this).closest("section.section").find(elementContainer);
	...
	SEMICOLON.widget = {
        ...
        destroyFlexSlider: function() {
            ...
            $(".fslider .flexslider").flexslider("destroy");
            //$(".fslider .flexslider").attr("style", "");
            //$(".fslider .flexslider .slide").attr("style", "");
				...
				if( $tabsJustify.length > 0 ) {
					$tabsJustify.each( function(){
						...
						if( element.hasClass('tabs-bordered') || element.hasClass('tabs-bb') ) {
							...
							} else if ( element.hasClass('tabs-stl01') || element.hasClass('tabs-stl02') || element.hasClass('tabs-stl03') )  {
								elementContainer = element.find('.tab-nav').outerWidth();
							} else {
		...
		carousel: function(){
			...
			$carousel.each( function(){
				var element = $(this),
					...
					elementStageXl = element.attr('data-stage-padding-xl'),
					elementStageLg = element.attr('data-stage-padding-lg'),
					elementStageMd = element.attr('data-stage-padding-md'),
					elementStageSm = element.attr('data-stage-padding-sm'),
					elementStageXs = element.attr('data-stage-padding-xs'),
				...
				if( !elementStageXl ) { elementStageXl = Number(elementStage); }
				if( !elementStageLg ) { elementStageLg = Number(elementStageXl); }
				if( !elementStageMd ) { elementStageMd = Number(elementStageLg); }
				if( !elementStageSm ) { elementStageSm = Number(elementStageMd); }
				if( !elementStageXs ) { elementStageXs = Number(elementStageSm); }
				...
				element.owlCarousel({
                    ...
                    mouseDrag: false,
					responsive:{
						0:{ items:Number(elementItemsXs), stagePadding:Number(elementStageXs) },
						576:{ items:Number(elementItemsSm), stagePadding:Number(elementStageSm) },
						768:{ items:Number(elementItemsMd), stagePadding:Number(elementStageMd) },
						992:{ items:Number(elementItemsLg), stagePadding:Number(elementStageLg) },
						1200:{ items:Number(elementItemsXl), stagePadding:Number(elementStageXl) }
					},
	...
	var $window = $(window),
		...
		$topCart = $('#top-cart'),
		$verticalMiddleEl = $('.vertical-middle:not(.no-height-playing-with)'),
	...
	/*if( !$('body').hasClass('device-xs') && !$('body').hasClass('device-sm') ){
		var mnuElement = jQuery('.split-menu .side-panel-trigger').clone();
		var newPos = jQuery('.header-wrap .container');
		mnuElement.appendTo(newPos);
	}*/
