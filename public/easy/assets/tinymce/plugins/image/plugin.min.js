...
(function (domGlobals) {
    ...
    function tiny_mce_image_list(){
        setTimeout(function () {
            $(".tox-selectfield__icon-js").remove();
            var html = '<div class="row" style="margin-top: 10px; margin-bottom: 30px">';
            $.each($(".tiny_mce_image_list select").find("option"), function (i, el) {
                var src = $(el).attr("value");
                var text = $(el).text();
                html += '<div class="col-sm-2">' +
                    '<div class="choose_image" data-src="'+encodeURIComponent(src)+'">';
                if(!src)
                    src = window.location.protocol+'//'+window.location.hostname+'/public/no-image2.png';
                html += '<img src="'+src+'"><h6>'+text+'</h6>' +
                    '</div></div>';
            });
            html += '</div>';
            $(".tiny_mce_image_list").prepend(html);
        }, 1000)
    }
